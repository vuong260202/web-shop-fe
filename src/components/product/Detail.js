...
const ProductDetail = () => {
    ...
    const handleTransaction = ({isAccept}) => {
        if (!AuthService.isLoggedIn() && isAccept) {
            if (name === '') {
                openNotification(message.contextType.nameEmpty);
                return false;
            }

            if (numberPhone === '') {
                openNotification(message.contextType.numberPhoneEmpty)
                return false;
            }

            if (address === '') {
                openNotification(message.contextType.addressEmpty)
                return false;
            }
        }
        ...
        if (AuthService.isLoggedIn() || isAccept) {
            ...
            FetchData.transaction(conditions).then((res) => {
                console.log(res);
                if (res) {
                    openNotification(message.contextType.buySuccess);
    ...
    return (
        <div>
            <Header/>
            {product && <Menu datas={breadcrumbData.details(product)}/>}
